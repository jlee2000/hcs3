...
(* This problem set is a short program that prints out a survey and,
   ...
   OCaml in action.  Posting a message to the server is optional, and
   is only available as an exercise to those students who have an
   @college.harvard.edu email address (required for authentication).
   All other students should leave the post flag set to false.
...
(* 1.f. Set this variable to true if you want your information,
   ...
   http://pset0.herokuapp.com.  Reminder: only available for students
   with @college.harvard.edu email addresses. *)
let post : bool = false;;
